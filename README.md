...
Этот компонент работает с инфоблоками (рассматривается как замена модуля веб-форм). Поля формы берутся из свойств указанного инфоблока и результаты формы сохраняются в этот же инфоблок. Результаты формы так же отправляются на e-mail администратора и на e-mail, заполненный в форме (если имеется свойство с символьным кодом "email" и оно было заполнено). Шаблоны этих e-mail-ов хранятся элементами другого инфоблока, специально для шаблонов писем (см. инструкции ниже).
